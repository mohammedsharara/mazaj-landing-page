...
*ملاحظة الضمان:* القطعة خاضعة لضمان الاستبدال المجاني في حال اخترتم المقاس ولم يناسبني تماماً.`;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/75 backdrop-blur-sm animate-in fade-in duration-200">
      <div
        className="bg-white dark:bg-[#141414] border border-neutral-200 dark:border-[#2B2B2B] rounded-3xl w-full max-w-lg overflow-hidden shadow-2xl relative flex flex-col max-h-[92vh] transition-colors"
        role="dialog"
        aria-modal="true"
        aria-labelledby="order-modal-title"
      >
        {/* Header */}
        <div className="p-4 sm:p-5 border-b border-neutral-200 dark:border-[#262626] flex items-center justify-between bg-neutral-50 dark:bg-[#181818]">
          <div className="flex items-center gap-2.5">
            <div className="p-2 rounded-xl bg-amber-50 dark:bg-[#FFA000]/15 text-[#D97706] dark:text-[#FFA000]">
          ...
          <button
            onClick={onClose}
            className="p-1.5 rounded-lg text-neutral-400 hover:text-neutral-900 dark:text-[#A5A5A5] dark:hover:text-[#F7F4EF] hover:bg-neutral-100 dark:hover:bg-[#222222] transition-colors cursor-pointer"
        ...
        {/* Modal Form Content */}
        <form onSubmit={handleSubmitToWhatsApp} className="p-4 sm:p-6 overflow-y-auto space-y-4">
          ...
          <div>
            ...
            <div className="flex gap-3 items-center">
              <div className="w-14 h-14 rounded-xl overflow-hidden shrink-0 border border-neutral-200 dark:border-[#333333] bg-neutral-100 dark:bg-[#202020]">
                <img
              ...
              <div className="flex-1">
                <select
                  ...
                  className="w-full bg-neutral-50 dark:bg-[#1A1A1A] border border-neutral-200 dark:border-[#2D2D2D] rounded-xl px-3.5 py-2.5 text-xs sm:text-sm text-neutral-900 dark:text-[#F7F4EF] font-bold focus:outline-none focus:ring-2 focus:ring-[#FFA000]"
                >
                  {PRODUCTS.map((p) => (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                {currentItem.priceOptions.plain && (
                  ...
                  >
                    ...
                    <div className="text-sm font-black text-neutral-900 dark:text-[#F7F4EF]">{currentItem.priceOptions.plain}</div>
                  </button>
                )}
                ...
                {currentItem.priceOptions.singleSide && (
                  ...
                  >
                    ...
                    <div className="text-sm font-black text-neutral-900 dark:text-[#F7F4EF]">{currentItem.priceOptions.singleSide}</div>
                  </button>
                )}
                ...
                {currentItem.priceOptions.bothSides && (
                  ...
                  >
                    ...
                    <div className="text-sm font-black text-neutral-900 dark:text-[#F7F4EF]">{currentItem.priceOptions.bothSides}</div>
                  </button>
                )}
                ...
                {currentItem.priceOptions.fullPrint && (
                  ...
                  >
                    ...
                    <div className="text-sm font-black text-neutral-900 dark:text-[#F7F4EF]">{currentItem.priceOptions.fullPrint}</div>
                  </button>
                )}
              </div>
            </div>
            ...
            <div className="flex flex-wrap gap-2">
              {availableColors.map((c) => (
                <button
                  ...
                  key={c}
                  onClick={() => setColor(c)}
                  className={`px-3 py-1.5 rounded-xl border text-xs cursor-pointer font-bold transition-all ${
                    color === c
          ...
          <div className="p-3.5 rounded-2xl bg-amber-50/50 dark:bg-[#1B1B1B] border border-amber-200/70 dark:border-[#333333] space-y-2.5">
            ...
            <div className="grid grid-cols-2 gap-2.5">
              <div>
                <label htmlFor="modal-height" className="block text-[11px] font-bold text-neutral-700 dark:text-[#A5A5A5] mb-1 flex items-center gap-1">
                  <Ruler className="w-3 h-3 text-[#FFA000]" />
                  <span>طولك (سم):</span>
                </label>
                <input
                  id="modal-height"
                  ...
                  className="w-full bg-white dark:bg-[#151515] border border-neutral-200 dark:border-[#2C2C2C] rounded-xl px-3 py-2 text-xs font-bold text-neutral-900 dark:text-[#F7F4EF] focus:outline-none focus:ring-2 focus:ring-[#FFA000]"
                />
              </div>
              ...
              <div>
                <label htmlFor="modal-weight" className="block text-[11px] font-bold text-neutral-700 dark:text-[#A5A5A5] mb-1 flex items-center gap-1">
                  <Weight className="w-3 h-3 text-[#FFA000]" />
                  <span>وزنك (كغ):</span>
                </label>
                <input
                  id="modal-weight"
            ...
            <label className="flex items-start gap-2 pt-1 cursor-pointer">
              <input
                ...
                className="mt-0.5 rounded text-[#FFA000] focus:ring-[#FFA000]"
              />
            ...
            <div className="flex flex-wrap gap-1.5 max-h-24 overflow-y-auto p-1">
              {availableSizes.map((s) => (
                <button
                  ...
                  key={s}
                  onClick={() => setSize(s)}
                  className={`px-3 py-1.5 rounded-xl font-['Plus_Jakarta_Sans',sans-serif] font-bold text-xs flex items-center justify-center transition-all cursor-pointer ${
                    size === s
          ...
          <div>
            ...
            <textarea
              ...
              rows={2}
              className="w-full bg-neutral-50 dark:bg-[#1A1A1A] border border-neutral-200 dark:border-[#2D2D2D] rounded-xl p-3 text-xs text-neutral-900 dark:text-[#F7F4EF] placeholder-neutral-400 focus:outline-none focus:ring-2 focus:ring-[#FFA000]"
            />
          </div>
          ...
          <div className="grid grid-cols-2 gap-3 pt-1">
            <div>
              ...
              <input
                ...
                className="w-full bg-neutral-50 dark:bg-[#1A1A1A] border border-neutral-200 dark:border-[#2D2D2D] rounded-xl px-3 py-2 text-xs text-neutral-900 dark:text-[#F7F4EF] placeholder-neutral-400 focus:outline-none focus:ring-2 focus:ring-[#FFA000]"
              />
            </div>
            <div>
